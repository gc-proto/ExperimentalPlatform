...
listen() {
  ...
  # remove the ONLY_FULL_GROUP_BY from sql_mode 
  sed '/.*mysqld_safe.*/a sql_mode = "STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION"' -i /etc/mysql/mysql.conf.d/mysqld.cnf
  # remove the ONLY_FULL_GROUP_BY from sql_mode 
  sed '/.*mysqld.*/a sql_mode = "STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION"' -i /etc/mysql/mysql.conf.d/mysqld.cnf
...
initialize_mysql_database() {
  ...
  if [ ! -d ${MYSQL_DATA_DIR}/mysql ]; then
    ...
    #echo "Set global condition ..."
    #mysql -uroot \
    #-v -e "SET GLOBAL sql_mode=(SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY',''));"
    ...
    if [ -n "${DB_REMOTE_ROOT_NAME}" -a -n "${DB_REMOTE_ROOT_HOST}" ]; then
      ...
      mysql -uroot \
      -e "CREATE USER '${DB_REMOTE_ROOT_NAME}'@'${DB_REMOTE_ROOT_HOST}' IDENTIFIED BY '${DB_REMOTE_ROOT_PASS}';GRANT ALL PRIVILEGES ON *.* TO '${DB_REMOTE_ROOT_NAME}'@'${DB_REMOTE_ROOT_HOST}'; FLUSH PRIVILEGES;"
    fi
      ...
      for db in $(awk -F',' '{for (i = 1 ; i <= NF ; i++) print $i}' <<< "${DB_NAME}"); do
        ...
        mysql --defaults-file=/etc/mysql/debian.cnf \
          ...
          if [ -n "${DB_USER}" ]; then
            ...
            echo "Create remote user \"${DB_USER}"\
            # Create the Remote USER to allow to access from any IP address
            mysql --defaults-file=/etc/mysql/debian.cnf \
...
install_sql_dump() {
    ...
    do
      ...
      # run the install specific to this indexer
      echo "Installing sql dump file Dump20191029_awr_schema.sql ..."
      # mysql --defaults-file=/etc/mysql/debian.cnf  awr_schema < Dump20191029_awr_schema.sql
      mysql --defaults-file=/etc/mysql/debian.cnf -uroot awr_schema\
       < Dump20191029_awr_schema.sql
      echo "Completed dump"
